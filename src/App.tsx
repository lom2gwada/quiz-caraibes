...
function AppInner({ profile, onProfileChange, session }: { profile: Profile | null; onProfileChange: (p: Profile) => void; session: Session | null }) {
  ...
  return <main className="app-shell">
    ...
    {view === 'atlas' && dataset && <AtlasPage rows={dataset.rows} schema={dataset.schema} shapes={dataset.shapes} region={dataset.region} regionViewBox={dataset.regionViewBox} capitalColumn={dataset.capitalColumn} i18n={dataset.i18n} onOpenFiche={setFicheSubject} onOpenMap={dataset.region ? () => navigate('map') : undefined} onBack={() => navigate('start')} />}
