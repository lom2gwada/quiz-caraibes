...
const SESSION_KEY = 'quiz-forge:guest-pass'
...
export async function checkGuestPass(token: string): Promise<boolean> {
  const { data, error } = await supabase.rpc('check_quiz_forge_guest_pass', { p_token: token })
...
export async function deleteGuestPass(token: string): Promise<void> {
  const { error } = await supabase.from('quiz_forge_guest_passes').delete().eq('token', token)
  if (error) throw error
}
